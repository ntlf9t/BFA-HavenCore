...
void WildBattlePetMgr::UnreplaceCreature(Creature* creature)
{
    ...
    Unit* unit = ObjectAccessor::GetCreatureOrPetOrVehicle(*creature, pTemplate->ReplacedRelation[creature->GetGUID()]);
    if (!unit || !unit->ToCreature())
        return;
...
void WildBattlePetMgr::LeaveBattle(Creature* creature, bool p_Defeated)
{
    ...
    Creature* originalCreature = unit->ToCreature();
    UnreplaceCreature(originalCreature);

    if (p_Defeated)
    {
        originalCreature->SetRespawnTime(originalCreature->GetCreatureData() ? originalCreature->GetCreatureData()->spawntimesecs : 15);
        originalCreature->ForcedDespawn();
        originalCreature->RemoveCorpse(false);
    }
}
