...
void WorldSession::HandleBattlemasterJoinOpcode(WorldPackets::Battleground::BattlemasterJoin& battlemasterJoin)
{
    ...
    else
    {
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
        {
            ...
            {
                WorldPackets::Battleground::BattlefieldStatusFailed battlefieldStatus;
                sBattlegroundMgr->BuildBattlegroundStatusFailed(&battlefieldStatus, bg, member, 0, 0, err, &errorGuid);
...
{
    if (battlefieldPort.Ticket.RequesterGuid != _player->GetGUID() || battlefieldPort.Ticket.Type != WorldPackets::LFG::RideType::Battlegrounds)
    {
        TC_LOG_DEBUG("bg.battleground", "CMSG_BATTLEFIELD_PORT %s rejected invalid ticket requester/type. Requester: %s, Type: %u",
            GetPlayerInfo().c_str(), battlefieldPort.Ticket.RequesterGuid.ToString().c_str(), uint32(battlefieldPort.Ticket.Type));
        return;
    }
...
void WorldSession::HandleBattlemasterJoinArena(WorldPackets::Battleground::BattlemasterJoinArena& packet)
{
    ...
    for (GroupReference* itr = grp->GetFirstMember(); itr != nullptr; itr = itr->next())
    {
        ...
        {
            WorldPackets::Battleground::BattlefieldStatusFailed battlefieldStatus;
                sBattlegroundMgr->BuildBattlegroundStatusFailed(&battlefieldStatus, bg, member, 0, arenatype, err, &errorGuid);
            member->SendDirectMessage(battlefieldStatus.Write());
            continue;
        }
