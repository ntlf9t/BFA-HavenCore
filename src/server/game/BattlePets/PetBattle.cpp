...
uint32 PetBattleTeam::GetCatchAbilityID()
{
    ...
    if (auto player = ObjectAccessor::GetObjectInWorld(PlayerGuid, static_cast<Player*>(nullptr)))
    {
        uint32 trapLevel = std::max<uint32>(1, std::min<uint32>(player->GetBattlePetTrapLevel(), 3));
    ...
    for (size_t currentTeamID = 0; currentTeamID < MAX_PET_BATTLE_TEAM; ++currentTeamID)
    {
        ...
        if (BattleType == PET_BATTLE_TYPE_PVE && PveBattleType == PVE_BATTLE_PET_WILD)
        {
            for (size_t i = 0; i < Teams[currentTeamID]->TeamPetCount; ++i)
            {
                ...
                Unit* wildUnit = ObjectAccessor::FindUnit(currentPet->OriginalCreature);
                Creature* wildPet = wildUnit ? wildUnit->ToCreature() : nullptr;
                if (!wildPet)
                    continue;

                wildPet->RemoveUnitFlag(UnitFlags(UNIT_FLAG_PACIFIED | UNIT_FLAG_IMMUNE_TO_PC));
