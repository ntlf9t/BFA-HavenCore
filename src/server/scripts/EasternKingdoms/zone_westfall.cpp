...
public:
  ...
  struct npc_thugAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *who) override {
      ...
      if (who->ToPlayer()->GetQuestStatus(QUEST_LOUS_PARTING_THOUGHTS) ==
      ...
      if (!bSummoned) {
        for (uint8 i = 0; i < 4; ++i) {
          ...
          if (Creature *thug = me->SummonCreature(
                  NPC_THUG, ThugPositions[i],
                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
      ...
      if (SummonTimer < diff) {
        if (bSummoned) {
          if (Player *player = ObjectAccessor::GetPlayer(*me, PlayerGUID)) {
            ...
            case 2: {
              if (Creature *thug = GetThug(1))
                thug->Say("She wanted me to tell you that she appreciates the "
                          "job that we did for her on the Furlbrows. Gave me a "
                          "pile o'gold to split with you all.",
                          LANG_UNIVERSAL);
              SummonTimer = 7000;
              Phase++;
              break;
            ...
            case 5: {
              if (Creature *thug = GetThug(0))
                thug->Say("Whoa, what do we have here? Looks like we have "
            ...
            case 6: {
              if (Creature *thug = GetThug(0))
                thug->Say("Only one thing to do with a louisy, good-for-nothin "
                          "eavesdropper.",
                          LANG_UNIVERSAL);
              SummonTimer = 4500;
              Phase++;
...
public:
  ...
  struct npc_horatioAI : public ScriptedAI {
    ...
    void StartSummon() {
      if (!bSummoned) {
        if (Creature *Investigator01 = me->SummonCreature(
                NPC_INVESTIGATOR2, -9854.414f, 916.481f, 30.100f, 5.3867f,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 66000)) {
          Investigator01GUID = Investigator01->GetGUID();
          bSummoned1 = true;
        }
        if (Creature *Investigator02 = me->SummonCreature(
                NPC_INVESTIGATOR2, -9854.252f, 906.651f, 29.833f, 0.730950f,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 66000)) {
    ...
    void UpdateAI(uint32 diff) override {
      if (TextTimer < diff) {
        if (bSummoned) {
          if (Player *player = ObjectAccessor::GetPlayer(*me, PlayerGUID))
            if (Creature *Investigator01 =
                    ObjectAccessor::GetCreature(*me, Investigator01GUID))
              if (Creature *Investigator02 =
                      ObjectAccessor::GetCreature(*me, Investigator02GUID)) {
                switch (Phase) {
                case 0: {
                  Investigator01->Say(
                ...
                case 1: {
                  Investigator01->Say("Given the body temperature. I'd say "
                                      "they've been dead no more than 6 hours.",
                                      LANG_UNIVERSAL);
                ...
                case 3: {
                  ...
                  me->SetWalk(true);
                  me->GetMotionMaster()->MovePoint(0, -9852.267f, 911.928f,
                                                   30.028f);
                  me->GetMotionMaster()->MovePoint(1, -9851.928f, 909.8602f,
                                                   29.931f);
                  me->SetStandState(UNIT_STAND_STATE_KNEEL);
                  TextTimer = 6000;
                  Phase++;
                ...
                case 4: {
                  me->Say("Looks like they really put the cart.",
                          LANG_UNIVERSAL);
                  TextTimer = 5000;
                  Phase++;
                  break;
                ...
                case 6: {
                  ...
                  me->GetMotionMaster()->MovePoint(0, -9852.267f, 911.928f,
                                                   30.028f);
                  me->GetMotionMaster()->MovePoint(1, -9849.818f, 914.904f,
                                                   30.27f);
                  me->GetMotionMaster()->MovePoint(2, -9849.86f, 914.859f,
                                                   30.268f);
                  me->SetWalk(false);
                  TextTimer = 8000;
                  Phase++;
...
#define GOSSIP_HELLO_DRIFTER2                                                  \
  "Maybe a couple copper will loosen your tongue. Now tell me, did you see "   \
  "who killed the Furlbrows?"
...
public:
  ...
  bool OnGossipHello(Player *player, Creature *creature) override {
    if (player->GetQuestStatus(26209) == QUEST_STATUS_INCOMPLETE)
      AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_HELLO_DRIFTER1,
                       GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF + 1);

    if (player->GetQuestStatus(26209) == QUEST_STATUS_INCOMPLETE)
      AddGossipItemFor(player, GOSSIP_ICON_CHAT, GOSSIP_HELLO_DRIFTER2,
  ...
  bool OnGossipSelect(Player *player, Creature *creature, uint32 /*sender*/,
    ...
    if (action == GOSSIP_ACTION_INFO_DEF + 1) {
      switch (rand() % 7) { // Say needs moved to creature_texts
      case 0: {
        creature->Say(
            "Listen, pal. I don't want any trouble, ok? I didn't see who "
            "murdered 'em, but I sure heard it! Lots of yelling. Human "
            "voices... you dig? Now get out of here before I change my mind "
            "about beating you up and takin' your shoes.",
            LANG_UNIVERSAL);
      ...
      case 1: {
        creature->Say("I didn't see who killed 'm, bub/sis, but I got a whiff. "
                      "Smelled rich, kinda like you. Damn shame too. Furlbrows "
                      "were a fixture around here. Nice people, always willin' "
                      "to share a meal or a patch of dirt.",
                      LANG_UNIVERSAL);
      ...
      case 2: {
        creature->Say("Who killed the Furlbrows? I'll tell you who killed the "
                      "Furlbrows: KING VARIAN WRYNN. THAT'S WHO! And he's "
                      "killin' the rest of us too. One bum at a time. The only "
                      "thing I can tell you is that I saw some gnolls leavin' "
                      "the place a few hours before the law arrived.",
                      LANG_UNIVERSAL);
      ...
      case 3: {
        creature->Say(
            "Between you, me, and the tree, murlocs killed the Furlbrows. Yep, "
            "saw 'em with my own two eyes. Think they'd been casin' the joint "
            "for days, maybe months. They left in a hurry once they got wind "
            "of 'Johnny Law' and the idiot brigade over there...",
            LANG_UNIVERSAL);
    ...
    if (action == GOSSIP_ACTION_INFO_DEF + 2) {
      if (!player->HasEnoughMoney(uint64(2))) {
        ...
        case 1: {
          creature->Say("I didn't see who killed 'm, bub/sis, but I got a "
                        "whiff. Smelled rich, kinda like you. Damn shame too. "
                        "Furlbrows were a fixture around here. Nice people, "
                        "always willin' to share a meal or a patch of dirt.",
                        LANG_UNIVERSAL);
          player->KilledMonsterCredit(CREDIT_SAY2);
          creature->DespawnOrUnsummon(5000);
          break;
        ...
        case 2: {
          creature->Say(
              "Who killed the Furlbrows? I'll tell you who killed the "
              "Furlbrows: KING VARIAN WRYNN. THAT'S WHO! And he's killin' the "
              "rest of us too. One bum at a time. The only thing I can tell "
              "you is that I saw some gnolls leavin' the place a few hours "
        ...
        case 3: {
          creature->Say(
              "Between you, me, and the tree, murlocs killed the Furlbrows. "
              "Yep, saw 'em with my own two eyes. Think they'd been casin' the "
              "joint for days, maybe months. They left in a hurry once they "
              "got wind of 'Johnny Law' and the idiot brigade over there...",
              LANG_UNIVERSAL);
          player->KilledMonsterCredit(CREDIT_SAY4);
          creature->DespawnOrUnsummon(5000);
          break;
        ...
        case 4: {
          creature->Say(
              "I wonder if it's possible to eat rocks? Got plenty of rocks "
              "around here. Just imagine it! I'd be the richest person in the "
              "world for making that discovery!",
              LANG_UNIVERSAL);
        ...
        case 5: {
          creature->Say("Looks like I found us a savory and clean piece of "
                        "dirt! Tonight we eat like kings, Mr. Penguin! Of "
                        "course I'll share it with you! You're my best friend!",
                        LANG_UNIVERSAL);
          creature->SetReactState(REACT_AGGRESSIVE);
          creature->AI()->AttackStart(player);
          break;
        ...
        case 6: {
          creature->Say("HAHAHAH! Good one, Mr. Penguin! GOOD ONE!",
                        LANG_UNIVERSAL);
          creature->SetReactState(REACT_AGGRESSIVE);
          creature->AI()->AttackStart(player);
          break;
        ...
        case 7: {
          creature->Say(
              "What happened to me? I used to be the king of Stormwind!",
              LANG_UNIVERSAL);
          creature->SetReactState(REACT_AGGRESSIVE);
          creature->AI()->AttackStart(player);
          break;
...
enum eCrate { QUEST_LIVIN_THE_LIFE = 26228 };
...
public:
  ...
  struct npc_crate_mineAI : public ScriptedAI {
    ...
    void PartySummoned() {
      if (!bSummoned) {
        if (Creature *glubtok1 =
                me->SummonCreature(42492, -9848.11f, 1395.29f, 37.70f, 0.56f,
                                   TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 85000))
          if (Creature *shadowy1 = me->SummonCreature(
                  42515, -9826.083f, 1406.738f, 36.885f, 3.56f,
                  TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 85000)) {
    ...
    void UpdateAI(uint32 diff) override {
      if (EntryTime <= diff) {
        if (bSummoned) {
          // if (Player* player = ObjectAccessor::GetPlayer(*me, PlayerGUID))
          if (Creature *glubtok1 =
                  ObjectAccessor::GetCreature(*me, GlubtoklGUID))
            if (Creature *shadowy1 =
                    ObjectAccessor::GetCreature(*me, Shadowy1GUID)) {
              switch (Phase) {
              case 0: {
                EntryTime = 2500;
                Phase++;
              ...
              case 1: {
                ...
                shadowy1->GetMotionMaster()->MovePoint(0, -9840.526f, 1399.37f,
                                                       37.177f);
                EntryTime = 7000;
                Phase++;
                break;
              ...
              case 2: {
                glubtok1->Say("What little human want? Why you call Glubtok?",
                              LANG_UNIVERSAL);
                EntryTime = 5000;
                Phase++;
                break;
              ...
              case 3: {
                shadowy1->Say(
                    ...
                    "Running two bit extortion operations out of a cave?",
                    LANG_UNIVERSAL);
                EntryTime = 8500;
                Phase++;
                break;
              ...
              case 5: {
                shadowy1->Say("Oh will you? Do you dare cross that line and "
                              "risk your life?",
                              LANG_UNIVERSAL);
                EntryTime = 6500;
                Phase++;
              ...
              case 6: {
                shadowy1->Say("You may attempt to kill me - and fail - or you "
                              "make take option two.",
                              LANG_UNIVERSAL);
                EntryTime = 6500;
                Phase++;
                break;
              ...
              case 12: {
                shadowy1->Say("I will call you when the dawning is upon us.",
                              LANG_UNIVERSAL);
                EntryTime = 6000;
                Phase++;
                break;
              ...
              case 13: {
                ...
                for (std::list<Player *>::const_iterator itr = players.begin();
                     itr != players.end(); ++itr)
...
public:
  ...
  struct npc_shadowy_triggerAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *who) override {
      ...
      if (who->ToPlayer()->GetQuestStatus(QUEST_THE_DAWNING_OF_NEW_DAY) ==
          QUEST_STATUS_INCOMPLETE) {
        if (who->IsWithinDistInMap(me, 20.0f) && !bSummoned) {
          PlayerGUID = who->GetGUID();
          StartSpeech();
        }
      } else if (who->ToPlayer()->GetQuestStatus(QUEST_A_VISION_OF_THE_PAST) ==
                 QUEST_STATUS_INCOMPLETE) {
        if (who->IsWithinDistInMap(me, 2.0f) && !bSummoned) {
          PlayerGUID = who->GetGUID();
          me->TextEmote(
              "Follow the trail of homeless to the Deadmines dungeon entrance.",
              NULL, true);
        }
      }
    }
    ...
    void StartSpeech() {
      if (!bSummoned) {
        if (Creature *Shadowy2 = me->SummonCreature(
                NPC_SHADOWY2, -11016.31f, 1478.82f, 47.80f, 2.016f,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000)) {
    ...
    void SummonCrowd() {
      if (!bSummoned1) {
        if (Creature *Witness1 = me->SummonCreature(
                NPC_LISTENER, -11009.036f, 1490.47f, 43.58f, 4.16f,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
          if (Creature *Witness2 = me->SummonCreature(
                  NPC_LISTENER, -11010.76f, 1488.21f, 43.57f, 4.33f,
                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
            if (Creature *Witness3 = me->SummonCreature(
                    NPC_LISTENER, -11017.39f, 1491.76f, 43.19f, 4.78f,
                    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
              if (Creature *Witness4 = me->SummonCreature(
                      NPC_LISTENER, -11021.73f, 1493.054f, 43.184f, 5.09f,
                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
                if (Creature *Witness5 = me->SummonCreature(
                        NPC_LISTENER, -11025.74f, 1487.70f, 43.17f, 5.45f,
                        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
                  if (Creature *Witness6 = me->SummonCreature(
                          NPC_LISTENER, -11025.25f, 1482.23f, 43.03f, 6.04f,
                          TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
                    if (Creature *Witness7 = me->SummonCreature(
                            NPC_LISTENER, -11029.68f, 1481.255f, 43.185f, 6.20f,
                            TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
                      if (Creature *Witness8 = me->SummonCreature(
                              NPC_LISTENER, -11024.46f, 1473.88f, 43.02f, 0.43f,
                              TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000))
                        if (Creature *Witness9 = me->SummonCreature(
                                NPC_LISTENER, -11019.49f, 1471.70f, 43.21f,
                                1.09f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                90000))
                          if (Creature *Witness10 = me->SummonCreature(
                                  NPC_LISTENER, -11021.56f, 1497.053f, 43.20f,
                                  5.00f, TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                  90000))
                            if (Creature *Witness11 = me->SummonCreature(
                                    NPC_LISTENER, -11012.29f, 1488.14f, 43.77f,
                                    4.16f,
                                    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                    90000))
                              if (Creature *Witness12 = me->SummonCreature(
                                      NPC_LISTENER, -11014.67f, 1493.14f,
                                      43.23f, 4.60f,
                                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                      90000))
                                if (Creature *Witness13 = me->SummonCreature(
                                        NPC_LISTENER, -11019.8f, 1494.25f,
                                        43.2f, 4.77f,
                                        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                        90000))
                                  if (Creature *Witness14 = me->SummonCreature(
                                          NPC_LISTENER, -11023.6f, 1489.35f,
                                          43.17f, 4.77f,
                                          TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                          90000))
                                    if (Creature *Witness15 = me->SummonCreature(
                                            NPC_LISTENER, -11023.1f, 1482.51f,
                                            43.07f, 6.25f,
                                            TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                            90000))
                                      if (Creature *Witness16 = me->SummonCreature(
                                              NPC_LISTENER, -11027.2f, 1494.37f,
                                              43.17f, 5.15f,
                                              TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                              90000))
                                        if (Creature *Witness17 =
                                                me->SummonCreature(
                                                    NPC_LISTENER, -11029.6f,
                                                    1488.29f, 43.19f, 5.64f,
                                                    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                                    90000))
                                          if (Creature *Witness18 =
                                                  me->SummonCreature(
                                                      NPC_LISTENER, -11030.8f,
                                                      1485.13f, 43.31f, 5.94f,
                                                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                                      90000))
                                            if (Creature *Witness19 =
                                                    me->SummonCreature(
                                                        NPC_LISTENER, -11026.2f,
                                                        1478.62f, 42.94f, 6.17f,
                                                        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                                        90000))
                                              if (Creature *Witness20 =
                                                      me->SummonCreature(
                                                          NPC_LISTENER,
                                                          -11013.2f, 1497.81f,
                                                          43.31f, 4.44f,
    ...
    void UpdateAI(uint32 diff) override {
      if (SummonTimer < diff) {
        if (bSummoned) {
          if (Player *player = ObjectAccessor::GetPlayer(*me, PlayerGUID))
            if (Creature *Shadowy2 =
                    ObjectAccessor::GetCreature(*me, Shadowy2GUID)) {
              switch (Phase) {
              case 0: {
              ...
              case 2: {
                Shadowy2->Yell("Brothers. Sisters. We are ABANDONED - the "
                               "orphaned children of Stormwind.",
                               LANG_UNIVERSAL);
                SummonTimer = 5500;
                Phase++;
                break;
              ...
              case 3: {
                Shadowy2->Yell("Our 'king' sits atop his throne made of gold "
                               "and shrugs at our plight!",
                               LANG_UNIVERSAL);
                SummonTimer = 4000;
                Phase++;
                break;
              ...
              case 4: {
                Shadowy2->Yell("Meanwhile, our children die of starvation on "
                               "these very streets!",
                               LANG_UNIVERSAL);
                SummonTimer = 4500;
                Phase++;
              ...
              case 5: {
                Shadowy2->Yell("HIS war, not ours, cost us our livelihood. WE "
                               "paid for the Alliance's victories with our "
                               "blood and the blood of our loved ones!",
                               LANG_UNIVERSAL);
                SummonTimer = 4500;
                Phase++;
                break;
              ...
              case 6: {
                Shadowy2->Yell("The time has come, brothers and sisters, to "
                               "stop this injustice!",
                               LANG_UNIVERSAL);
                SummonTimer = 4500;
                Phase++;
              ...
              case 7: {
                Shadowy2->Yell(
                    "The government of Stormwind, of the ALLIANCE, must be "
                    "made accountable for what it has done to us!",
                    LANG_UNIVERSAL);
                SummonTimer = 4500;
                Phase++;
                break;
              ...
              case 8: {
                Shadowy2->Yell(
                    "Today, we are reborn! Today, we take a stand as men and "
                    "women, not nameless, faceless numbers!",
                    LANG_UNIVERSAL);
                SummonTimer = 4500;
                Phase++;
                break;
              ...
              case 9: {
                if (Creature *listener =
                        me->FindNearestCreature(NPC_LISTENER, 35.0f, true)) {
                  if (!bText) {
                    me->TextEmote("<homeless people applaud and cheer>", NULL,
                                  true);
                    listener->TextEmote(
                        TEXT_EMOTE_APPLAUD,
                        ObjectAccessor::GetPlayer(*me, PlayerGUID), false);
                    bText = true;
                  }
...
public:
  ...
  struct npc_shadowy_towerAI : public ScriptedAI {
    ...
    void MoveInLineOfSight(Unit *who) override {
      ...
      if (who->ToPlayer()->GetQuestStatus(QUEST_SECRETS_OF_THE_TOWER) ==
          QUEST_STATUS_INCOMPLETE) {
    ...
    void StartEvent() {
      if (!bSumm) {
        if (!bSumm1) {
          if (Creature *Shadowy3 = me->SummonCreature(
                  42662, -11138.659f, 545.20f, 70.30f, 0.19f,
                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000)) {
            Shadowy3GUID = Shadowy3->GetGUID();
            Shadowy3->AddUnitFlag(UNIT_FLAG_NOT_SELECTABLE);
            Shadowy3->SetWalk(true);
            Shadowy3->GetMotionMaster()->MovePoint(0, -11131.710f, 546.810f,
                                                   70.380f);
            bSumm1 = true;
          }
        }
        ...
        if (!bSumm2) {
          if (Creature *Glubtok3 = me->SummonCreature(
                  42492, -11128.11f, 547.52f, 70.41f, 3.32f,
                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 90000)) {
    ...
    void UpdateAI(uint32 diff) override {
      if (SummonTimer < diff) {
        if (bSumm) {
          if (Player *player = ObjectAccessor::GetPlayer(*me, PlayerGUID))
            if (Creature *Glubtok3 =
                    ObjectAccessor::GetCreature(*me, Glubtok3GUID))
              if (Creature *Shadowy3 =
                      ObjectAccessor::GetCreature(*me, Shadowy3GUID)) {
                switch (Phase) {
                case 0: {
                ...
                case 1: {
                  Glubtok3->Say("The gnolls have failed, mistress.",
                                LANG_UNIVERSAL);
                  SummonTimer = 6000;
                  Phase++;
                  break;
                ...
                case 2: {
                  Shadowy3->Say("They provided the distraction I required. We "
                                "continue as planned.",
                                LANG_UNIVERSAL);
                  SummonTimer = 6000;
                  Phase++;
                ...
                case 3: {
                  Glubtok3->Say("But mistress, the admiral is sti...",
                                LANG_UNIVERSAL);
                  SummonTimer = 6000;
                  Phase++;
                  break;
                ...
                case 4: {
                  Shadowy3->Say("We will free the admiral during the dawning.",
                                LANG_UNIVERSAL);
                  SummonTimer = 6000;
                  Phase++;
                  break;
                ...
                case 6: {
                  Shadowy3->Say("Judgment day is soon upon us, Helix.",
                                LANG_UNIVERSAL);
                  SummonTimer = 6000;
                  Phase++;
                  break;
                ...
                case 7: {
                  Shadowy3->Say("Call for the people. I wish to speak to them "
                                "one last time before the dawning.",
                                LANG_UNIVERSAL);
                  SummonTimer = 6000;
                  Phase++;
                  break;
...
public:
  ...
  bool OnQuestAccept(Player *player, Creature *creature,
    ...
    if (quest->GetQuestId() == QUEST_RISE_OF_THE_BROTHERHOOD) {
      ENSURE_AI(npc_rise_br::npc_rise_brAI, creature->AI())->PlayerGUID =
          player->GetGUID();

      if (!(ENSURE_AI(npc_rise_br::npc_rise_brAI, creature->AI())
                ->EventStarted)) {
        ENSURE_AI(npc_rise_br::npc_rise_brAI, creature->AI())->EventStarted =
            true;
      }
    }
    return true;
  ...
  struct npc_rise_brAI : public ScriptedAI {
    ...
    void SummonBrotherHood() {
      if (Creature *Thief1 = me->SummonCreature(
              NPC_THIEF, -10500.37f, 1042.65f, 60.51f, 3.06f,
              TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
        if (Creature *Thief2 = me->SummonCreature(
                NPC_THIEF, -10500.99f, 1046.73f, 60.517f, 3.29f,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
          if (Creature *Thief3 = me->SummonCreature(
                  NPC_THIEF, -10505.202f, 1040.46f, 60.51f, 1.88f,
                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
            if (Creature *Thief4 = me->SummonCreature(
                    NPC_THIEF, -10507.89f, 1039.52f, 60.51f, 1.95f,
                    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
              if (Creature *Thief5 = me->SummonCreature(
                      NPC_THIEF, -10513.54f, 1038.66f, 60.51f, 0.51f,
                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
                if (Creature *Thief6 = me->SummonCreature(
                        NPC_THIEF, -10514.930f, 1042.012f, 60.51f, 0.399f,
                        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
                  if (Creature *Thief7 = me->SummonCreature(
                          NPC_THIEF, -10516.797f, 1048.61f, 59.95f, 5.32f,
                          TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
                    if (Creature *Thief8 = me->SummonCreature(
                            NPC_THIEF, -10514.032f, 1049.80f, 59.92f, 5.11f,
                            TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
                      if (Creature *Thief9 = me->SummonCreature(
                              NPC_THIEF, -10509.012f, 1051.92f, 59.85f, 5.11f,
                              TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
                        if (Creature *Thief10 = me->SummonCreature(
                                NPC_THIEF, -10504.77f, 1053.57f, 59.86f, 4.84f,
                                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000))
                          if (me->SummonCreature(
                                  42755, -10506.12f, 1053.13f, 59.10f, 4.92f,
                                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                  120000))
                            if (me->SummonCreature(
                                    42753, -10509.368f, 1057.28f, 57.86f, 4.95f,
                                    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                    120000)) {
    ...
    void RemoveStun() {
      if (Creature *Horatio =
              me->FindNearestCreature(NPC_HORATIO2, 50.0f, true))
        if (Creature *Danuvin =
                me->FindNearestCreature(NPC_DANUVIN, 50.0f, true))
          if (Creature *Guard = me->FindNearestCreature(NPC_GUARD, 50.0f, true))
            if (Creature *Investigator =
                    me->FindNearestCreature(NPC_INVESTIGATOR, 50.0f, true))
              if (Creature *Investigator2 =
                      me->FindNearestCreature(NPC_INVESTIGATOR3, 50.0f, true)) {
    ...
    void SummonFireTrigger() {
      if (Creature *Fire1 =
              me->SummonCreature(45937, -10507.5f, 1050.6f, 60.5189f, 3.06f,
                                 TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000))
        if (Creature *Fire2 = me->SummonCreature(
                45937, -10502.0f, 1051.81f, 67.7637f, 3.29f,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000))
          if (Creature *Fire3 = me->SummonCreature(
                  45937, -10504.9f, 1038.74f, 68.2617f, 1.88f,
                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000))
            if (Creature *Fire4 = me->SummonCreature(
                    45937, -10507.4f, 1039.3f, 68.1306f, 1.95f,
                    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000))
              if (Creature *Fire5 = me->SummonCreature(
                      45937, -10512.0f, 1040.95f, 60.5179f, 0.51f,
                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000))
                if (Creature *Fire6 = me->SummonCreature(
                        45937, -10516.2f, 1043.94f, 70.1173f, 0.399f,
                        TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000))
                  if (Creature *Fire7 = me->SummonCreature(
                          45937, -10501.6f, 1039.39f, 71.2453f, 5.32f,
                          TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000))
                    if (Creature *Fire8 = me->SummonCreature(
                            45937, -10520.0f, 1053.13f, 57.8726f, 5.11f,
                            TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000))
                      if (Creature *Fire9 = me->SummonCreature(
                              45937, -10504.0f, 1063.19f, 56.481f, 5.11f,
                              TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000))
                        if (Creature *Fire10 = me->SummonCreature(
                                45937, -10492.4f, 1048.5f, 67.612f, 4.84f,
                                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 50000))
                          if (Creature *Fire11 = me->SummonCreature(
                                  45937, -10503.8f, 1033.96f, 60.6086f, 4.92f,
                                  TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                  50000))
                            if (Creature *Fire12 = me->SummonCreature(
                                    45937, -10502.4f, 1037.98f, 102.67f, 4.95f,
                                    TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                    50000))
                              if (Creature *Fire13 = me->SummonCreature(
                                      45937, -10528.7f, 1038.45f, 56.4513f,
                                      4.95f,
                                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT,
                                      50000)) {
        ...
        if (EventTimer < diff) {
          if (Player *player = ObjectAccessor::GetPlayer(*me, PlayerGUID))
            if (Creature *Hope = me->FindNearestCreature(NPC_HOPE, 50.0f, true))
              if (Creature *Ripsnarl =
                      me->FindNearestCreature(NPC_RIPSNARL, 50.0f, true)) {
                switch (Phase) {
                case 0: {
                  me->SummonCreature(
                      NPC_HORATIO2, -10508.0f, 1047.54f, 60.518f, 0.0f,
                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000);
                  me->SummonCreature(
                      NPC_DANUVIN, -10510.0f, 1040.67f, 60.518f, 0.0f,
                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000);
                  me->SummonCreature(
                      NPC_GUARD, -10504.7f, 1042.87f, 60.518f, 0.0f,
                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000);
                  me->SummonCreature(
                      NPC_INVESTIGATOR, -10509.1f, 1048.56f, 60.518f, 0.0f,
                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000);
                  me->SummonCreature(
                      NPC_INVESTIGATOR3, -10506.3f, 1047.7f, 60.518f, 0.0f,
                      TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 100000);
                  std::string l_Msg = "I don't like this, " +
                ...
                case 1: {
                  ...
                  Hope->GetMotionMaster()->MovePoint(0, -10507.65f, 1042.81f,
                                                     60.51f);
                  EventTimer = 8000;
                  Phase++;
                  break;
                ...
                case 2: {
                  Hope->Say("You bastards will burn for what you did.",
                            LANG_UNIVERSAL);
                  EventTimer = 4000;
                  Phase++;
                  break;
                ...
                case 4: {
                  Hope->Say("Hope? Is that what I was supposed to feel when I "
                            "saw my father decapitated by your henchmen?",
                            LANG_UNIVERSAL);
                  EventTimer = 12000;
                  Phase++;
                  break;
                ...
                case 5: {
                  Hope->Say("Hope is a cruel joke, played upon us by a harsh "
                            "and uncaring world. There is no Hope, there is "
                            "only Vanessa, Vanessa VanCleef.",
                            LANG_UNIVERSAL);
                  EventTimer = 9500;
                  Phase++;
                  break;
                ...
                case 7: {
                  Hope->Yell("RISE UP BROTHERHOOD! THE DAWNING DAY IS UPON US!",
                             LANG_UNIVERSAL);
                  SummonBrotherHood();
                  EventTimer = 4000;
                  Phase++;
                ...
                case 10: {
                  Hope->GetMotionMaster()->MovePoint(1, -10512.36f, 1044.36f,
                                                     60.518f);
                  EventTimer = 3000;
                  Phase++;
                  break;
                ...
                case 14: {
                  ...
                  Ripsnarl->GetMotionMaster()->MovePoint(2, -10513.41f,
                                                         1041.11f, 60.518f);
                  EventTimer = 2500;
                  Phase++;
                  break;
                ...
                case 15: {
                  Ripsnarl->GetMotionMaster()->MovePoint(3, -10511.34f,
                                                         1042.46f, 60.5172f);
                  EventTimer = 1500;
                  Phase++;
                  break;
                ...
                case 16: {
                  ...
                  std::string l_Msg =
                      "And you, " + std::string(player->GetName()) +
                      ".  I will spare your life. You have done much to help "
                      "our cause, albeit unwittingly, but the next time we "
                      "meet it will be as enemies.";
                  Hope->Say(l_Msg, LANG_UNIVERSAL);
                  Ripsnarl->GetMotionMaster()->MovePoint(4, -10516.64f,
                                                         1064.78f, 55.362f);
                  EventTimer = 8000;
                  Phase++;
                  break;
                ...
                case 17: {
                  ...
                  Hope->GetMotionMaster()->MovePoint(5, -10513.37f, 1056.48f,
                                                     57.605f);
                  me->Say("Why'd you have the Furlbrows killed?",
                          LANG_UNIVERSAL);
                  EventTimer = 4500;
                  Phase++;
                  break;
                ...
                case 18: {
                  ...
                  Hope->Say(
                      "I had no choice, lieutenant. They recognized me. The "
                      "only people in the world who even knew I existed, "
                      "recognized my face from when I was an infant.",
                      LANG_UNIVERSAL);
                  EventTimer = 7000;
                  Phase++;
                  break;
                ...
                case 19: {
                  Hope->Say("I took no pleasure in their deaths.",
                            LANG_UNIVERSAL);
                  // SummonFireTrigger(); // I cannot get this to work properly,
                  // leaving code in cae someone else works it out
                  EventTimer = 5000;
                  Phase++;
                  break;
                }
                case 20: {
                  Hope->Yell("Leave nothing but ashes in your wake, brothers! "
                             "Burn Sentinel Hill in the ground!",
                             LANG_UNIVERSAL);
                  EventTimer = 4000;
                  Phase++;
                  break;
                ...
                case 21: {
                  std::string l_Msg = std::string(player->GetName()) +
                                      ", get to Stormwind. Tell King Wrynn "
                                      "everything, EVERYTHING! GO NOW!";
                  me->Say(l_Msg, LANG_UNIVERSAL);
                  Hope->GetMotionMaster()->MovePoint(6, -10518.38f, 1067.99f,
                                                     54.84f);
                  Ripsnarl->SetFacingToObject(Hope);

                  if (Creature *Helix2 =
                          me->FindNearestCreature(42753, 30.0f, true)) {
                    Helix2->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
                    Helix2->GetMotionMaster()->MovePoint(7, -10520.01f,
                                                         1062.31f, 55.386f);
                  }

                  if (Creature *Glubtok4 =
                          me->FindNearestCreature(42755, 30.0f, true)) {
                    Glubtok4->AddUnitMovementFlag(MOVEMENTFLAG_WALKING);
                    Glubtok4->GetMotionMaster()->MovePoint(8, -10511.79f,
                                                           1065.78f, 55.085f);
                  }
                ...
                case 22: {
                  ...
                  EventTimer = 100;
                  if (Creature *Helix2 =
                          me->FindNearestCreature(42753, 40.0f, true)) {
                    Helix2->DespawnOrUnsummon();
                  }

                  if (Creature *Glubtok4 =
                          me->FindNearestCreature(42755, 40.0f, true)) {
      ...
      if (Defias_DoFire) {
        if (Creature *pTarget = me->FindNearestCreature(45937, 30.0f, true)) {
          if (ThrowTimer < diff) {
            ...
            ThrowTimer = 4000;
            // me->SetWalk(true);
            // me->GetMotionMaster()->MovePoint(0, me->GetPositionX()-5,
            // me->GetPositionY()+5, me->GetPositionZ());
          } else
            ThrowTimer -= diff;
        }
      }
...
public:
  ...
  struct npc_fire_triggerAI : public ScriptedAI {
    ...
    void Reset() override { me->CastSpell(me, 71025, true); }
...
public:
  ...
  struct npc_summonerAI : public ScriptedAI {
    ...
    void Reset() override { me->CastSpell(me, 64446, true); }
  };
};

enum eInvestigate { NPC_INVESTIGATOR4 = 42559, NPC_HOME = 42386 };
...
public:
  ...
  struct npc_horatio_investigateAI : public ScriptedAI {
    ...
    void StartSummon() {
      if (!bSummonn) {
        if (Creature *Investigator04 = me->SummonCreature(
                NPC_INVESTIGATOR4, -9856.825f, 1279.25f, 40.96f, 3.29f,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 66000)) {
        ...
        if (Creature *Home = me->SummonCreature(
                NPC_HOME, -9862.88f, 1278.52f, 40.80f, 0.43f,
                TEMPSUMMON_TIMED_DESPAWN_OUT_OF_COMBAT, 66000)) {
    ...
    void UpdateAI(uint32 diff) override {
      if (TextTimer < diff) {
        if (bSummonn) {
          if (Creature *Investigator04 =
                  ObjectAccessor::GetCreature(*me, Investigator04GUID))
            if (Creature *Home = ObjectAccessor::GetCreature(*me, HomeGUID)) {
              switch (Phase) {
              case 0: {
                Investigator04->Say("You were standing right here! What the "
                                    "hell did you see?Speak up!",
                                    LANG_UNIVERSAL);
                TextTimer = 4000;
                Phase++;
                break;
              ...
              case 1: {
                Home->Say("I... I didn't see nothin'! He...he died of natural "
                          "causes.",
                          LANG_UNIVERSAL);
                TextTimer = 6000;
                Phase++;
              ...
              case 2: {
                Investigator04->Say("Natural causes? Two bullets in the chest "
                                    "and his shoes are on his head. What kind "
                                    "of natural death would that be?",
                                    LANG_UNIVERSAL);
                TextTimer = 6000;
                Phase++;
                break;
              ...
              case 4: {
                me->Say("This was an execution. Whoever did this was sending a "
                        "message,,,",
                        LANG_UNIVERSAL);
                TextTimer = 5000;
                Phase++;
              ...
              case 5: {
                me->Say("A message for anyone that would dare snitch on these "
                        "cryminals.",
                        LANG_UNIVERSAL);
                TextTimer = 4000;
                Phase++;
              ...
              case 6: {
                me->Say("It would appear that poor Lou really put this foot...",
                        LANG_UNIVERSAL);
                TextTimer = 4000;
                Phase++;
                break;
...
enum eLousPartingThoughts {
  ...
  EVENT_LPT_THUG_SAY2 = 4,
  EVENT_LPT_THUG_SAY3 = 5,
  EVENT_LPT_THUG_SAY4 = 6,
  EVENT_LPT_THUG_SAY5 = 7,
  EVENT_LPT_THUG_SAY6 = 8,
  EVENT_LPT_THUG_CREDIT = 9,
  EVENT_LPT_THUG_SHOOT1 = 10,
  EVENT_LPT_THUG_SHOOT2 = 11,
  EVENT_LPT_THUG_SCREAM = 12,
  EVENT_LPT_THUG_WARNING = 13,
  EVENT_LPT_HUSTLER_SAY0 = 14,
  ...
  THUG_SAY_6 = 6,
  // Trigger (42562)
  TRIGGER_SAY_WARNING = 0,
  // Hustler (42390)
  HUSTLER_SAY_0 = 0,
  HUSTLER_SAY_1 = 1,
};

Position const ThugSpawnPos[4] = {
...
public:
  at_westfall_two_shoed_lou_thugs()
      : AreaTriggerScript("at_westfall_two_shoed_lou_thugs") {}

  bool OnTrigger(Player *player, AreaTriggerEntry const * /*at*/,
                 bool /*entered*/) override {
    if (!player->IsAlive())
      return true;
    if (player->GetQuestStatus(QUEST_LOUS_PARTING_THOUGHTS_NEW) !=
        QUEST_STATUS_INCOMPLETE)
      return true;
    if (Creature *trigger = player->FindNearestCreature(
            NPC_LOUS_PARTING_THOUGHTS_TRIGGER, 50.0f, true))
      trigger->AI()->SetGUID(player->GetGUID(), 0);
    return true;
  }
};

// NPC 42387 - Westfall Thug (summoned by trigger creature)
struct npc_westfall_thug : public ScriptedAI {
  npc_westfall_thug(Creature *creature) : ScriptedAI(creature) {}

  void Reset() override {
    // Start passive until the scene reaches its attack phase
    me->RestoreFaction();
    me->SetReactState(REACT_PASSIVE);
    me->AddUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
  }
...
struct npc_westfall_lous_parting_thoughts_trigger : public ScriptedAI {
  ...
  // Called from area trigger when a qualifying player enters
  ...
  void SetData(uint32 /*type*/, uint32 data) override {
    if (data == DATA_THUG_DEATH) {
      ...
      if (_thugDeathCount >= 4) {
        _events.ScheduleEvent(EVENT_LPT_THUG_CREDIT, 0);
  ...
  void UpdateAI(uint32 diff) override {
    if (!_eventInvokerGUID.IsEmpty()) {
      ...
      if (!invoker ||
          invoker->GetQuestStatus(QUEST_LOUS_PARTING_THOUGHTS_NEW) ==
              QUEST_STATUS_NONE) {
        _events.Reset();
        _eventInvokerGUID.Clear();
    ...
    while (uint32 eventId = _events.ExecuteEvent()) {
      ...
      switch (eventId) {
      case EVENT_LPT_SUMMON_THUGS:
        for (uint8 i = 0; i < 4; ++i) {
          if (Creature *thug =
                  me->SummonCreature(NPC_WESTFALL_THUG, ThugSpawnPos[i],
      ...
      case EVENT_LPT_THUG_SAY0: {
        Creature *thug1 = ObjectAccessor::GetCreature(*me, _summonGUIDs[0]);
        if (invoker && thug1)
          thug1->AI()->Talk(THUG_SAY_0, invoker);
        _events.ScheduleEvent(EVENT_LPT_THUG_SAY1, 5000);
        break;
      }
      case EVENT_LPT_THUG_SAY1: {
        Creature *thug2 = ObjectAccessor::GetCreature(*me, _summonGUIDs[1]);
        if (invoker && thug2)
          thug2->AI()->Talk(THUG_SAY_1, invoker);
        _events.ScheduleEvent(EVENT_LPT_THUG_SAY2, 5000);
        break;
      }
      case EVENT_LPT_THUG_SAY2: {
        Creature *thug2 = ObjectAccessor::GetCreature(*me, _summonGUIDs[1]);
        if (invoker && thug2)
          thug2->AI()->Talk(THUG_SAY_2, invoker);
        _events.ScheduleEvent(EVENT_LPT_THUG_SAY3, 8500);
        break;
      }
      case EVENT_LPT_THUG_SAY3: {
        Creature *thug3 = ObjectAccessor::GetCreature(*me, _summonGUIDs[2]);
        if (invoker && thug3)
          thug3->AI()->Talk(THUG_SAY_3, invoker);
        _events.ScheduleEvent(EVENT_LPT_THUG_SAY4, 5000);
        break;
      }
      case EVENT_LPT_THUG_SAY4: {
        Creature *thug1 = ObjectAccessor::GetCreature(*me, _summonGUIDs[0]);
        if (invoker && thug1) {
          thug1->AI()->Talk(THUG_SAY_4, invoker);
          for (ObjectGuid const &guid : _summonGUIDs)
            if (Creature *thug = ObjectAccessor::GetCreature(*me, guid))
              thug->SetFacingToObject(invoker);
        }
        _events.ScheduleEvent(EVENT_LPT_THUG_SAY5, 8500);
        break;
      }
      case EVENT_LPT_THUG_SAY5: {
        Creature *thug1 = ObjectAccessor::GetCreature(*me, _summonGUIDs[0]);
        if (invoker && thug1)
          thug1->AI()->Talk(THUG_SAY_5, invoker);
        _events.ScheduleEvent(EVENT_LPT_THUG_SAY6, 5000);
        break;
      }
      case EVENT_LPT_THUG_SAY6: {
        Creature *thug1 = ObjectAccessor::GetCreature(*me, _summonGUIDs[0]);
        if (invoker && thug1) {
          thug1->AI()->Talk(THUG_SAY_6, invoker);
          for (ObjectGuid const &guid : _summonGUIDs) {
            if (Creature *thug = ObjectAccessor::GetCreature(*me, guid)) {
              // Make thugs attackable and start combat
              thug->SetReactState(REACT_AGGRESSIVE);
              thug->RemoveUnitFlag(UNIT_FLAG_NON_ATTACKABLE);
              thug->SetFaction(14);
              thug->AddUnitFlag(UNIT_FLAG_PVP_ATTACKABLE);
      ...
      case EVENT_LPT_THUG_CREDIT: {
        // Grant credit to the invoker
        if (invoker)
          invoker->CastSpell(invoker, SPELL_HOBO_INFORMATION, true);

        // Also grant credit to other nearby players with the same quest
        std::list<Player *> nearPlayers;
        me->GetPlayerListInGrid(nearPlayers, 75.0f);
        for (Player *nearPlayer : nearPlayers) {
          if (nearPlayer == invoker)
            continue;
          if (nearPlayer->GetQuestStatus(QUEST_LOUS_PARTING_THOUGHTS_NEW) !=
              QUEST_STATUS_INCOMPLETE)
            continue;
      ...
      case EVENT_LPT_THUG_SHOOT1:
        if (invoker)
          me->PlayDistanceSound(SOUND_SHOOTING, invoker);
        _events.ScheduleEvent(EVENT_LPT_THUG_SHOOT2, 1200);
        break;

      case EVENT_LPT_THUG_SHOOT2:
        if (invoker)
          me->PlayDistanceSound(SOUND_SHOOTING, invoker);
        _events.ScheduleEvent(EVENT_LPT_THUG_SCREAM, 1200);
        break;

      case EVENT_LPT_THUG_SCREAM:
        if (invoker)
          me->PlayDistanceSound(SOUND_SCREAM, invoker);
        _events.ScheduleEvent(EVENT_LPT_THUG_WARNING, 2500);
        break;

      case EVENT_LPT_THUG_WARNING:
        if (invoker)
          Talk(TRIGGER_SAY_WARNING, invoker);
        break;
...
public:
  at_westfall_small_time_hustler()
      : AreaTriggerScript("at_westfall_small_time_hustler") {}
  ...
  bool OnTrigger(Player *player, AreaTriggerEntry const * /*at*/,
    ...
    if (!player->IsAlive())
      return true;
    if (Creature *hustler =
            player->FindNearestCreature(NPC_SMALL_TIME_HUSTLER, 10.0f, true))
      hustler->AI()->SetGUID(player->GetGUID(), 0);
    return true;
  }
};
...
struct npc_westfall_small_time_hustler : public ScriptedAI {
  ...
  void SetGUID(ObjectGuid guid, int32 /*id*/) override {
    ...
    _eventInvokerGUID = guid;
    _events.ScheduleEvent(EVENT_LPT_HUSTLER_SAY0, 0);
  }
  ...
  void UpdateAI(uint32 diff) override {
    ...
    while (uint32 eventId = _events.ExecuteEvent()) {
      switch (eventId) {
      case EVENT_LPT_HUSTLER_SAY0: {
        Player *invoker = ObjectAccessor::GetPlayer(*me, _eventInvokerGUID);
        if (invoker) {
          me->SetFacingToObject(invoker);
          Talk(HUSTLER_SAY_0, invoker);
          _events.ScheduleEvent(EVENT_LPT_HUSTLER_SAY1, 4000);
        }
        break;
      }
      case EVENT_LPT_HUSTLER_SAY1: {
        Player *invoker = ObjectAccessor::GetPlayer(*me, _eventInvokerGUID);
        if (invoker) {
          Talk(HUSTLER_SAY_1, invoker);
          _events.ScheduleEvent(EVENT_LPT_HUSTLER_RESET, 60000);
        }
        break;
      }
      case EVENT_LPT_HUSTLER_RESET:
        _eventInvokerGUID = ObjectGuid::Empty;
        break;
